...
const navItems = [
  ...
  { id: "whitepaper", label: "Whitepaper", href: "#", external: false, hidden: false, isModal: true }, // Opens ForgenomicsModal
  { id: "forgenomics", label: "Forgonomics", href: "#", external: false, hidden: false, isModal: true },
...
const Navbar = ({ openForgenomicsModal, openFaqModal }) => { // Accept modal open functions as props
  const [mobileDrawerOpen, setMobileDrawerOpen] = useState(false);
  const [activeSection, setActiveSection] = useState("");
  // REMOVED: const [showWhitepaper, setShowWhitepaper] = useState(false); // This state is now managed in App.jsx
  // REMOVED: import WhitepaperModal from "./WhitepaperModal"; // No longer needed here
  ...
  const handleNavigation = (e, id) => {
    ...
    // Handle modal items
    if (navItem?.isModal) {
        if (id === "forgenomics" || id === "whitepaper") {
      ...
      <nav className="sticky top-0 z-50 py-3 backdrop-blur-lg bg-gray-900/60 border-b border-cyan-800/50">
        <div className="container px-4 mx-auto flex items-center justify-between relative">
          ...
          <ul className="hidden lg:flex flex-grow justify-between w-full space-x-4">
            ...
            {/* REMOVED: Old Whitepaper Modal Trigger, now handled by navItems */}
          </ul>
        ...
        {mobileDrawerOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-gray-900 bg-opacity-95 backdrop-blur-lg shadow-md rounded-b-md p-6">
            <ul className="space-y-6 flex flex-col items-center">
              ...
              {/* REMOVED: Old Whitepaper Button (Mobile), now handled by navItems */}
            </ul>
          </div>
        )}
      </nav>
      {/* REMOVED: Modal rendering from Navbar, App.jsx handles it */}
    </>
  );
};
